...
export async function generateStateReport(state, timePeriod = 'YoY') {
  try {
    ...
    // State Info Box
    doc.rect(80, 220, doc.page.width - 160, 120).fillAndStroke('#f0f9ff', colors.accent);
    ...
    const avgChange =
      walletShocks.reduce((sum, s) => sum + s.changePercent, 0) / walletShocks.length || 0;
    ...
    doc.rect(60, 380, doc.page.width - 120, 200).fillAndStroke(colors.lightGray, colors.border);
    ...
    doc
      ...
      .text(`${avgChange >= 0 ? '+' : ''}${avgChange.toFixed(1)}%`, 80, statY + 80, {
        width: statWidth - 40,
      });
    ...
    doc
      ...
      .text(`${increasingCount} / ${walletShocks.length}`, rightX, statY + 20, {
        width: statWidth - 40,
      });
    ...
    doc
      ...
      .text(`${maxChange >= 0 ? '+' : ''}${maxChange.toFixed(1)}%`, rightX, statY + 80, {
        width: statWidth - 40,
      });
    ...
    doc
      ...
      .fillColor(colors.gray)
      .text(
        'This report contains analysis of price changes across key consumer categories.',
        60,
        doc.page.height - 80,
    ...
    doc.fontSize(18).font('Helvetica-Bold').fillColor('#ffffff').text('Top Price Changes', 60, 15);
    ...
    walletShocks.forEach((shock, index) => {
      ...
      doc
        .rect(60, startY, doc.page.width - 120, 85)
        .fillAndStroke(index % 2 === 0 ? '#ffffff' : colors.lightGray, colors.border);

      // Rank badge
      doc.circle(80, startY + 20, 15).fill(colors.primary);
      doc
        .fontSize(12)
        .font('Helvetica-Bold')
        .fillColor('#ffffff')
    ...
    if (costDrivers.length > 0) {
      ...
      doc
        ...
        .font('Helvetica')
        .fillColor(colors.gray)
        .text('The following factors are contributing to price changes in your area:', 60, doc.y, {
      ...
      costDrivers.forEach((driver) => {
        ...
        // Label
        doc.fontSize(11).font('Helvetica-Bold').fillColor('#000').text(driver.label, 60, yPos);

        // Bar background
        doc.rect(60, yPos + 18, maxBarWidth, 24).fillAndStroke(colors.lightGray, colors.border);

        // Bar fill
        doc.rect(60, yPos + 18, barWidth, 24).fill(driver.color);
      ...
      doc
        ...
        .fillColor(colors.gray)
        .text(
          'Note: Percentages represent the relative contribution of each factor to overall price changes.',
          60,
          doc.y,
    ...
    doc
      ...
      .fillColor(colors.gray)
      .text(
        `Report generated by Dekleptocracy.com • © ${new Date().getFullYear()} • Data accurate as of ${new Date().toLocaleDateString()}`,
        60,
        doc.page.height - 70,
        { align: 'center', width: doc.page.width - 120 },
      );
